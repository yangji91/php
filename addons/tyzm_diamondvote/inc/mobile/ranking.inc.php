...
$rid=intval($_GPC['rid']);
$ty=$_GPC['ty'];
$reply = pdo_fetch("SELECT   rid,title,sharetitle,shareimg,sharedesc,config,addata,endtime,apstarttime,apendtime,status,description  FROM " . tablename($this->tablereply) . " WHERE rid = :rid ", array(':rid' => $rid));
