...
$reply = pdo_fetch("SELECT rid,title,topimg,eventrule,sharetitle,shareimg,sharedesc,addata,config,endtime,apstarttime,apendtime,status,description FROM " . tablename($this->tablereply) . " WHERE rid = :rid ORDER BY `id` DESC", array(':rid' => $rid));
