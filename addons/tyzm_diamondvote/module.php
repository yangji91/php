	...
	public function fieldsFormDisplay($rid = 0)
	{
		...
		if (empty($rid)) {
			...
			if ($issetmeal && empty($_W['isfounder'])) {
				...
				if (empty($meal)) {
					$setmealstatus = 1;
					$setmealmsg = "未开通套餐，请联系管理员开通套餐";
				}
				if ($meal['status']) {
					$setmealstatus = 1;
					$setmealmsg = "套餐已禁用，请联系管理员开通";
				}
				if ($meal['starttime'] > time()) {
					$setmealstatus = 1;
					$setmealmsg = "套餐未生效，请联系管理员开通";
				}
				if ($meal['endtime'] < time()) {
					$setmealstatus = 1;
					$setmealmsg = "套餐已过期，请联系管理员开通";
				}
